defmodule Explorer.ThirdPartyIntegrations.SourcifyFilePathBackfiller do
  ...
  def perform() do
    ...
    |> Enum.each(fn contract -> 
      with {:address_hash, address_hash_string} <- {:address_hash, "0x" <> Base.encode16(contract.address_hash.bytes, case: :lower)},
      ...
      %{
        ...
        |> Repo.all()
        |> Enum.each(fn source -> 
          new_name = if !is_nil(res = Enum.find(secondary_sources, fn src -> src["file_name"] =~ source.file_name end)), do: res["file_name"], else: nil
        ...
        _error ->
          Logger.debug(["Coudn't fetch file paths for #{"0x" <> Base.encode16(contract.address_hash.bytes, case: :lower)} from Sourcify"])
      end
    end)
  end
